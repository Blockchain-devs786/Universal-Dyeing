...
export const itemsService = {
  ...
  async delete(id: number) {
    ...
    let rows;
    try {
      rows = await sql`DELETE FROM items WHERE id = ${id} RETURNING id, name`;
    } catch (err: any) {
      if (err.message?.includes('foreign key constraint')) {
        throw new Error('Cannot delete this item because it is used in existing Inward/Outward records. Please disable it instead.');
      }
      throw err;
    }
    if (rows.length === 0) throw new Error('Item not found');
